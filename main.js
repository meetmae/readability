...
class Readability {
  ...
  colemanLiauIndex(text) {
    ...
    const sentences = Math.legacyRound(
      this.averageSentencePerWord(text) * 100,
      2
    );
  ...
  presentTense(word) {
    ...
    if (word.endsWith("ed")) {
      ...
      else return word.slice(0, -2); // assume we remove "ed"
    }
  ...
  difficultWords(text, syllableThreshold = 2) {
    ...
    for (let word of textList) {
      const normalized = this.presentTense(
        pluralize(word.toLocaleLowerCase(), 1)
      );
      // console.log(`difficultWords(${word}): norm=${normalized}, `
      //             `${this.syllableCount(word)} syllables, easy? ${easyWordSet.has(normalized)}`)
      if (
        !easyWordSet.has(normalized) &&
        this.syllableCount(word) >= syllableThreshold
      ) {
  ...
  daleChallReadabilityScore(text) {
    ...
    let score =
      0.1579 * difficultWords + 0.0496 * this.averageSentenceLength(text);
  ...
  gunningFog(text) {
    const perDiffWords =
      (this.difficultWords(text, 3) / this.lexiconCount(text)) * 100;
  ...
  textStandard(text, floatOutput = null) {
    ...
    lower = Math.legacyRound(
      this.daleChallToGrade(this.daleChallReadabilityScore(text))
    );
    upper = Math.ceil(
      this.daleChallToGrade(this.daleChallReadabilityScore(text))
    );
    ...
    const counterMap = [...new Set(grade)].map((x) => [
      x,
      grade.filter((y) => y === x).length,
    ]);
    ...
    return `${lowerScore}${Readability.getGradeSuffix(
      lowerScore
    )} and ${upperScore}${Readability.getGradeSuffix(upperScore)} grade`;
  }
  textMedian(text) {
    const grade = [];
    // Appending Flesch Kincaid Grade
    ...
    if (half & 0x1) return (grade[half - 1] + grade[half]) / 2;
